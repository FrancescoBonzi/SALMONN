"""
Evaluate Qwen2.5-Omni on MMAR benchmark.
...
def pretrained_prompt_template(batch, metadata, cfg):
    ...
    for i in range(len(batch["id"])):
        ...
        prompt = f"<|audio_bos|><|AUDIO|><|audio_eos|> {question} Select one option from the provided choices.\n{choices}"
...
def afthink_prompt_template(batch, metadata, cfg):
    ...
    for i in range(len(batch["id"])):
        ...
        prompt = f"<|audio_bos|><|AUDIO|><|audio_eos|> {question} Choose one among the following options:\n{choices}."
...
def afthink_no_reasoning_prompt_template(batch, metadata, cfg):
    ...
    for i in range(len(batch["id"])):
        ...
        prompt = f"<|audio_bos|><|AUDIO|><|audio_eos|> {question} Choose one among the following options:\n{choices}. Answer with the correct option."
        prompts.append(prompt)
...
def parse_args():
    ...
    parser.add_argument("--output-file", type=str, default="outputs/mmar/qwen_results.json", help="Output file")
...
def main():
    ...
    print("=" * 60)
    print("Qwen2.5-Omni MMAR Evaluation")
    print("=" * 60)
